...
def make_share(user, coeffs, ssss):
    ...
    # print(share.encode())
    return (user,share.encode())


def split(k, n, secret, ssss=False,random_id=[]):
    ...
    # c_0 is the secret.
    #
    # random_id = [i for i in range(1,n+1)]
    ...
    # args = [(i,coeffs,ssss) for i in random_id]
    # with Pool() as pool:
    #     result = pool.starmap(make_share,args)
    # return result
    return [make_share(i,coeffs,ssss) for i in random_id]
    # return [(i,make_share(i,coeffs,ssss)) for i in random_id]
