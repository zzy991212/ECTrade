...
INPUT_DIR = "/root/home/experiment/fairswap_data_onchain"
OUTPUT_DIR = "/root/home/experiment/fairswap_data_onchain_enc"
CSV_PATH = "/root/home/experiment/fairswap_data_enc_onchain_size.csv"
        ...
        for root, _, files in os.walk(INPUT_DIR):
            for file in files:
                ...
                try:
                    ...
                    # 2. 构建Merkle树
                    merkle_nodes, merkle_root = build_merkle_tree_with_all_nodes(blocks)
                    merkle_node_count = len(merkle_nodes)
                    print(f"处理{file}：块大小={block_size}，Merkle节点数={merkle_node_count}")
                    
                    # 3. 生成主密钥
                    k = generate_key(KEY_LENGTH)
                    
                    # 4. 加密文件块（核心修正：密钥长度与块大小一致）
                    encrypted_blocks = encrypt_items(blocks, k, block_size)
                    
                    # 5. 加密Merkle节点
                    encrypted_merkle_nodes = encrypt_items(merkle_nodes, k, MERKLE_NODE_SIZE)
                    ...
                    print(f"加密完成: {file}（大小={encrypted_size}字节）")
...
if __name__ == "__main__":
    if not os.path.isdir(INPUT_DIR):
        ...
        sys.exit(1)
    process_directory()
    print(f"加密文件保存至{OUTPUT_DIR}，大小记录在{CSV_PATH}")