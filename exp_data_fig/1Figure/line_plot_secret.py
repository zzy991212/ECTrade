...
import numpy as np
from bar_data import n, N, g_1_spl, g_1_rec, g_2_spl, g_2_rec, g_3_spl, g_3_rec, g_4_spl, g_4_rec, g_5_spl, g_5_rec
...
colors = ['#4BA97A', '#D45A00','#2C7ACD']  # Orange, Green, Red
...
for i, (rec_avg, rec_min, rec_max) in enumerate(zip(avg_rec, min_rec, max_rec)):
    line, = ax2.plot(x_ticks, rec_avg,
                     ...
                     marker=markers[i], markevery=1, markersize=18,
                     markeredgecolor='k', markeredgewidth=1.5,linewidth=5)
    lines.append(line)
    ax2.fill_between(x_ticks, rec_min, rec_max, color=colors[i], alpha=0.25)
...
for i, (spl_avg, spl_min, spl_max) in enumerate(zip(avg_spl, min_spl, max_spl)):
    ax1.plot(x_ticks, spl_avg,
             ...
             marker=markers[i], markevery=1, markersize=18,
             markeredgecolor='k', markeredgewidth=1.5,linewidth=5)
    ax1.fill_between(x_ticks, spl_min, spl_max, color=colors[i], alpha=0.25)
...
legend_elements = [
    Line2D([0], [0], linestyle='-', color=colors[0], lw=2.8, marker=markers[0],linewidth=3,
           markeredgecolor='k', markersize=15, label=f'N={selected_N[0]}'),
    Line2D([0], [0], linestyle='-', color=colors[1], lw=2.8, marker=markers[1],linewidth=3,
           markeredgecolor='k', markersize=15, label=f'N={selected_N[1]}'),
    Line2D([0], [0], linestyle='-', color=colors[2], lw=2.8, marker=markers[2],linewidth=3,
           markeredgecolor='k', markersize=15, label=f'N={selected_N[2]}'),

    Line2D([0], [0], linestyle='-', color='black', lw=2.8,linewidth=3,
           markeredgecolor='k', markersize=15, label=f'Split & Obfuscate'),
    Line2D([0], [0], linestyle='--', color='black', lw=2.8,linewidth=3,
           markeredgecolor='k', markersize=15, label=f'Reconstruct')
]
